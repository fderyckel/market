            ...
            fluidRow(
                ...
                tabBox(title = "Charts", width = 9, 
                       tabPanel(title = "Relative Performance", width = NULL, status = "warning", 
                                plotOutput("ggplot_etf", height = "600px")))), 
...
server <- function(input, output) {
    ...
    output$choose_region <- renderUI({
        selectInput("etf_region", "Choose your main focus", unique(etf_list$region))
    }) 
    
    output$pick_etf <- renderUI({
        ## If missing input, return to avoid error later in function
        if(is.null(input$etf_region))
            return()
        
        ## Get the data from the region
        etfs <- etf_list %>% 
            filter(region == input$etf_region) %>% .$ticker %>% sort()
        
        ## Create the check box
        checkboxGroupInput("columns", "Choose your countries", 
                           choices = etfs)
    })
